                ...
                if (ga.CurrentInspectorState == GameAnalyticsSDK.Setup.Settings.InspectorStates.Account)
                {
                    ...
                    if (!string.IsNullOrEmpty(ga.LoginStatus) && !ga.LoginStatus.Equals("Not logged in."))
                    {
                        ...
                        if (ga.JustSignedUp && !ga.HideSignupWarning)
                        {
                            ...
                            EditorGUILayout.HelpBox("Please be aware that our service might take a few minutes to get ready to receive events. Opened here to open Integration Status to follow the progress as you start sending events.", MessageType.Warning);
