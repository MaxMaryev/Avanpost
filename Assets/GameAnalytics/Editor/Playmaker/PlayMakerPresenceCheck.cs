		...
		if (_playmakerDetected)
		{
			...
			if (! _bridgeEnabled)
			{
				...
				if (EditorUtility.DisplayDialog("GameAnalytics : PlayMaker Detected","Do you want to enable PlayMaker Actions for GameAnalytics?","Yes","No"))
				{
					...
					EditorApplication.ExecuteMenuItem("Window/GameAnalytics/PlayMaker/_toggle Scripts");


				}else{
					EditorPrefs.SetBool(IgnorePlayMakerBridgeKey+"-"+Application.dataPath,true);
					Debug.Log("To enable PlayMaker support for GameAnalytics manualy, simply go to the menu: 'Window/GameAnalytics/PlayMaker/_toggle Scripts'");
				}
			}
		}
